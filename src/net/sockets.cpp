...
ssize_t sockets::write(int sockfd, const void *buf, int size) {
    return ::write(sockfd, buf, size);
}

ssize_t sockets::sendto(
    int sockfd, const void *buf, int len, const struct sockaddr *destAddr) {
...
void sockets::setReuseAddr(int sockfd, int on) {
    int optval = on ? 1 : 0;
    setsockopt(
        sockfd, SOL_SOCKET, SO_REUSEADDR, (const char *)&optval,
        sizeof(optval));
}
