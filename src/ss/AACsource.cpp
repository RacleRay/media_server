...
AACFileMeidaSource *
AACFileMeidaSource::createNew(UserEnvironment *env, const std::string &file) {
...
void AACFileMeidaSource::handleTask() {
    ...
    frame->mSize = getFrameFromAACFile(frame->mData, FRAME_MAX_SIZE);
    if (frame->mSize < 0) { return; }
    frame->mDataRef = frame->mData;
...
int AACFileMeidaSource::getFrameFromAACFile(uint8_t *buf, int size) {
    ...
    if (!parseAdtsHeader(tmpBuf, &mAdtsHeader)) { return -1; }

    if (mAdtsHeader.aacFrameLength > size) return -1;
