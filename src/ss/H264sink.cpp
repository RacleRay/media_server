...
H264FileSink *
H264FileSink::createNew(UserEnvironment *env, MediaSource *mediaSource) {
    if (!mediaSource) return NULL;
    return new H264FileSink(env, mediaSource);
}

H264FileSink::H264FileSink(UserEnvironment *env, MediaSource *mediaSource)
...
void H264FileSink::sendFrame(MediaFrame *frame) {
    ...
    uint8_t naluType = frame->mDataRef[0];

    if (frame->mSize <= RTP_MAX_PKT_SIZE) {
        memcpy(rtpHeader->payload, frame->mDataRef, frame->mSize);
        ...
        for (i = 0; i < pktNum; i++) {
             ...
             *   +---------------+
             * */
            //(naluType & 0x60)表示nalu的重要性，28表示为分片
            ...
            memcpy(
                rtpHeader->payload + 2, frame->mDataRef + pos,
                RTP_MAX_PKT_SIZE);
        ...
        if (remainPktSize > 0) {
            ...
            memcpy(
                rtpHeader->payload + 2, frame->mDataRef + pos, remainPktSize);
