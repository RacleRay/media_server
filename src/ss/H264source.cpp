...
H264FileMediaSource *
H264FileMediaSource::createNew(UserEnvironment *env, const std::string &file) {
    return new H264FileMediaSource(env, file);
}

H264FileMediaSource::H264FileMediaSource(
    UserEnvironment *env, const std::string &file)
    : MediaSource(env) {
    mSourceName = file;
...
void H264FileMediaSource::handleTask() {
    ...
    while (true) {
        frame->mSize = getFrameFromH264File(frame->mData, FRAME_MAX_SIZE);
        if (frame->mSize < 0) { return; }
        if (startCode3(frame->mData)) {
            startCodeNum = 3;
        } else {
            startCodeNum = 4;
        }
        frame->mDataRef = frame->mData + startCodeNum;
        frame->mSize -= startCodeNum;

        uint8_t naluType = frame->mDataRef[0] & 0x1F;
        // LOGI("startCodeNum=%d,naluType=%d,naluSize=%d", startCodeNum,
        // naluType, frame->mSize);
...
int H264FileMediaSource::getFrameFromH264File(uint8_t *frame, int size) {
    ...
    if (!startCode3(frame) && !startCode4(frame)) {
        ...
        LOGE(
            "Read %s error, no startCode3 and no startCode4",
            mSourceName.c_str());
        return -1;
    }
