...
RtspConnection *
RtspConnection::createNew(RtspServer *rtspServer, int clientFd) {
...
RtspConnection::RtspConnection(RtspServer *rtspServer, int clientFd)
    ...
    , mTrackId(MediaSession::TrackId::TrackNone)
    , mSessionId(rand())
    , mIsRtpOverTcp(false)
    , mStreamPrefix("track") {
...
RtspConnection::~RtspConnection() {
    ...
    for (int i = 0; i < MEDIA_MAX_TRACK_NUM; ++i) {
        if (mRtpInstances[i]) {
            ...
            if (!session) { session->removeRtpInstance(mRtpInstances[i]); }
            delete mRtpInstances[i];
        }

        if (mRtcpInstances[i]) { delete mRtcpInstances[i]; }
    }
}
...
void RtspConnection::handleReadBytes() {
    ...
    switch (mMethod) {
        case OPTIONS:
            if (!handleCmdOption()) goto disConnect;
            break;
        case DESCRIBE:
            if (!handleCmdDescribe()) goto disConnect;
            break;
        case SETUP:
            if (!handleCmdSetup()) goto disConnect;
            break;
        case PLAY:
            if (!handleCmdPlay()) goto disConnect;
            break;
        case TEARDOWN:
            if (!handleCmdTeardown()) goto disConnect;
            break;
...
bool RtspConnection::parseSetup(std::string &message) {
    ...
    if (mTrackId == MediaSession::TrackNone) { return false; }
...
bool RtspConnection::handleCmdOption() {
    ...
    if (sendMessage(mBuffer, strlen(mBuffer)) < 0) return false;
...
bool RtspConnection::handleCmdSetup() {
    ...
    if (session->isStartMulticast()) {
        snprintf(
            ...
            "\r\n",
            mCSeq,
            session->getMulticastDestAddr().c_str(), // 多播目标地址
        ...
        if (mIsRtpOverTcp) {
            ...
            // 创建 rtp over udp
            if (createRtpRtcpOverUdp(
                    mTrackId, mPeerIp, mPeerRtpPort, mPeerRtcpPort)
                != true) {
...
bool RtspConnection::handleCmdPlay() {
    ...
    for (int i = 0; i < MEDIA_MAX_TRACK_NUM; ++i) {
        if (mRtpInstances[i]) { mRtpInstances[i]->setAlive(true); }

        if (mRtcpInstances[i]) { mRtcpInstances[i]->setAlive(true); }
    }
...
bool RtspConnection::createRtpRtcpOverUdp(
    ...
    if (mRtpInstances[trackId] || mRtcpInstances[trackId]) return false;
    ...
    while (true) {
        ...
        if (mInputBuffer.readableBytes() < bufSize) {
            ...
            if (0x00 == rtpChannel) {
                ...
                LOGI(
                    "num=%d,rtcpHeader.packetType=%d,rtpSize=%d", num,
