...
MediaSession *MediaSession::createNewSession(std::string sessionName) {
    return new MediaSession(sessionName);
}

MediaSession::MediaSession(const std::string &sessionName)
    : mSessionName(sessionName), mIsMulticast(false) {
...
std::string MediaSession::generateSDPDescription() {
    ...
    for (int i = 0; i < MEDIA_MAX_TRACK_NUM; ++i) {
        ...
        if (mTracks[i].mIsAlive != true) continue;

        if (isStartMulticast()) port = getMulticastDestRtpPort((TrackId)i);
...
MediaSession::Track *MediaSession::getTrack(MediaSession::TrackId trackId) {
    for (int i = 0; i < MEDIA_MAX_TRACK_NUM; ++i) {
        if (mTracks[i].mTrackId == trackId) return &mTracks[i];
    }
...
bool MediaSession::addRtpInstance(
    ...
    Track *track = getTrack(trackId);
    if (!track || track->mIsAlive != true) return false;
...
bool MediaSession::removeRtpInstance(RtpInstance *rtpInstance) {
    for (int i = 0; i < MEDIA_MAX_TRACK_NUM; ++i) {
        ...
        std::list<RtpInstance *>::iterator it = std::find(
            mTracks[i].mRtpInstances.begin(), mTracks[i].mRtpInstances.end(),
            rtpInstance);
        if (it == mTracks[i].mRtpInstances.end()) continue;
...
void MediaSession::handleSendRtpPacket(
    MediaSession::Track *track, RtpPacket *rtpPacket) {
    //    LOGI("");
    ...
    for (it = track->mRtpInstances.begin(); it != track->mRtpInstances.end();
        ...
        if (rtpInstance->alive()) { rtpInstance->send(rtpPacket); }
    }
}
...
uint16_t MediaSession::getMulticastDestRtpPort(TrackId trackId) {
    if (trackId > TrackId1 || !mMulticastRtpInstances[trackId]) return -1;
